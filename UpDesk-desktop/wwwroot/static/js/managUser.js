...
async function fetchAndDisplayUsers() {
    ...
    const url = new URL(`${apiUrl}/api/usuarios`);
...
async function handleCreateUser(event) {
    ...
    try {
        // ATENÇÃO: Crie o endpoint POST /api/usuarios no seu backend.
        const response = await fetchWithAuth(`${apiUrl}/api/usuarios`, {
            method: 'POST',
...
async function handleEditUser(event) {
    ...
    try {
        ...
        const response = await fetchWithAuth(`${apiUrl}/api/usuarios/${id}`, {
            method: 'PUT',
            body: JSON.stringify(data)
        });
